...
int32_t main (int32_t argc, char *argv[])
{
    ...
    char *dir = dynprintf("%s%s%s", appdata, DSEP, "crystaleer");
#ifdef _WIN32
    mkdir(dir);
#else
    mkdir(dir, 0700);
    ...
    char *out = dynprintf("%s%s%s%s%s", appdata, DSEP, "crystaleer", DSEP, "stdout.txt");
    LOG_STDOUT = fopen(out, "w+");
    myfree(out);

    char *err = dynprintf("%s%s%s%s%s", appdata, DSEP, "crystaleer", DSEP, "stderr.txt");
